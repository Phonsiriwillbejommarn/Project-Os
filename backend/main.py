...
async def generate_overview_summary(data: dict):
    ...
    try:
        ...
        # Build prompt for AI - narrative style
        prompt = f"""คุณเป็น AI นักโภชนาการและโค้ชสุขภาพส่วนตัวที่เป็นมิตร สรุปภาพรวมสุขภาพวันนี้เป็นภาษาไทย เขียนแบบบรรยายเป็นย่อหน้าเดียว กระชับ อ่านง่าย เหมือนเพื่อนเล่าให้ฟัง

ข้อมูลผู้ใช้: {user_info.get('name', 'คุณ')} 
เป้าหมาย: {user_info.get('goal', 'MaintainWeight')}

โภชนาการวันนี้:
- กินไปแล้ว: {nutrition.get('calories_eaten', 0)} kcal (เป้า {nutrition.get('target_calories', 2000)} kcal)
...
def get_health_history(user_id: int, days: int = 7, db: Session = Depends(get_db)):
    ...
    for i in range(days):
        ...
        if metrics:
            hr_metrics = [m for m in metrics if m.heart_rate and m.heart_rate > 0]
